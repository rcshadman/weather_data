...
def clean(raw_data):
    ...
    for obj in raw_data['history']['observations']:
        tmp = {key: val for key, val in obj.items() if key in features_list and obj[
            key] not in ('', '-999', '-9999', '-9999.0', '-9999.00')}
...
def main(argv):
    try:
        ...
        if len(argv) < 2:
            print("Enter Sample like - python3 data_provider.py test ")
            sys.exit()
